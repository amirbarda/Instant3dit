import torch
from utils.geometry_utils import double_face_areas, calculate_vertex_normals
from utils.topology_utils import consolidate_indices
import igl
...
class RegularSampler:
    ...
    def create_sample_points(self, vs, faces, offset = 0):
        ...
        if offset != 0:
            face_first_vec = face_first_vec + face_first_vec/(torch.norm(face_first_vec, dim=-1, keepdim=True)+1e-8)*offset
            face_second_vec = face_second_vec + face_second_vec/(torch.norm(face_second_vec, dim=-1, keepdim=True)+1e-8)*offset
            face_first_vertex_dir = face_first_vertex - vs[:, faces[0, :]].mean(dim=2)
            face_first_vertex = face_first_vertex + face_first_vertex_dir/(torch.norm(face_first_vertex_dir, dim=-1, keepdim=True)+1e-8)*offset
